...
export function AISidebar({ emails }: { emails: any[] }) {
  ...
  const generateAIAnalysis = async (type: 'daily' | 'senders' | 'drafts') => {
    ...
    try {
      ...
      // Detailed logging of OpenAI API errors
      console.error('Error generating AI analysis:', error);
      
      // Log detailed error information including response status and body if available
      if (error.response) {
        console.error('[AISidebar] OpenAI API Error Details:', {
      ...
      // More descriptive error message for the user based on error type
      let errorDescription = "Could not generate AI analysis. Please try again later.";
      
      if (error.message?.includes('API key')) {
        errorDescription = "API key issue detected. Please check your OpenAI API key configuration.";
      } else if (error.response?.status === 429) {
        errorDescription = "OpenAI API rate limit exceeded. Please wait a moment and try again.";
      }
