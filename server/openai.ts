...
export function setupOpenAI(app: any, storage: IStorage) {
  ...
  app.get("/api/ai/daily-digest", async (req: Request, res: Response) => {
    ...
    console.log("[OpenAI] API Request received: /api/ai/daily-digest");
    
    try {
      if (!req.user) {
        console.log("[OpenAI] Unauthorized request to /api/ai/daily-digest");
      ...
      console.log(`[OpenAI] Daily digest generated successfully in ${requestDuration}ms`);
      
      return res.status(200).json(dailyDigest);
    } catch (error: any) {
      const requestDuration = Date.now() - requestStartTime;
      console.error(`[OpenAI] Error generating daily digest after ${requestDuration}ms:`, error);
